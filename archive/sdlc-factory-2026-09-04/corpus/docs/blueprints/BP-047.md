...
satisfies: [REQ-059]
covers: []
depends-on: [BP-002, BP-005, BP-014, BP-018, BP-019, BP-045, BP-058]
blocked-by: [BP-014]
rests-on:
  - claim: "A published page's question-and-answer section is available structurally on the draft row (`drafts.meta.faq`, written by BP-014 at generation) rather than only as prose inside `body_md`, so `FAQPage` markup is emitted from data and never from a render-time parse of markdown"
    disposition: refuted
    reason: "Architect's wave-close read, 2026-09-04 (rule 2.3b). The first half holds — `drafts.meta jsonb` is in BUILD.md §10's column list and BP-014's `## Data model delta` adopts §10's `drafts` verbatim. The second half is false of the corpus: **BP-014 writes nothing into `meta.faq`.** The string `faq` does not occur anywhere in BP-014 — not in its public interface, not in its data-model delta, not in its decisions — and BP-014's only stated addition to the row is `grounded_fact jsonb`. The three occurrences of `drafts.meta.faq` in the whole corpus are all in this node (this row, the `## Error & edge behavior` line and decision 2), so this node is reading a field no node commits to writing. That is a gap, not a contradiction, and it is recorded as a `blocked-by` on BP-014 with the reasoning in this node's body (rule 2.3) rather than resolved silently: whether the question-and-answer section becomes a structured field on the draft row is BP-014's to decide, because BP-014 owns generation. Nothing is broken today — no work order writes either side."
supersedes: []
code:
...
/** The cache tags this surface reads under, **declared in BP-045's
 *  `src/lib/publish/types.ts` and re-exported here unchanged** (ADR-092,
 *  2026-09-02) — the spelling every caller uses is the one below and did not
 *  change. BP-045's `attempt/` and BP-058's `destinations/hosted/` invalidate
 *  them; both are `src/lib/`, and importing them from this file would be the
 *  `src/lib → src/app` direction structure.md rule 6 forbids, while retyping
 *  the two literals would be a second home for a key whose copies diverge on
 *  the first rename (rule 2.4). This surface reads them and does not own them. */
//   tags: { site: (siteId: string) => `hosted:site:${string}`
//           page: (publicationId: string) => `hosted:page:${string}` }
//   — the shape, for reading; one re-export, never a second declaration.
export { tags } from '@/lib/publish/types'
```
...
2. **`FAQPage` is emitted from `drafts.meta.faq`, not from parsing markdown** —
   ...
   - **Blocked on BP-014, 2026-09-04** (rule 2.3 — the conflict is the edge,
     the reasoning is here). This decision says "the generator records the
     section structurally", and BP-014 is the generator. It does not: `faq`
     appears nowhere in BP-014 — not in its public interface, not in its
     `## Data model delta` (which adopts BUILD.md §10's `drafts` verbatim and
     adds `grounded_fact jsonb` alone), not in its decisions. Every occurrence
     of `drafts.meta.faq` in the corpus is in this node. So this node reads a
     field no node commits to writing, and criterion 3's markup has no data
     source. The edge is logged on this node, the later one, and it does not
     resolve the gap in BP-014's place: whether the question-and-answer
     section becomes a structured member of `drafts.meta` is BP-014's, because
     BP-014 owns generation and rule 7.1 gives one capability one owner.
     Nothing is broken today — no work order writes either side, and the
     absent-data behaviour above is already the correct render for a page
     with no such section, so the edge is a design gap and not a defect.
...
4. **The hosted cache tags are declared in `src/lib/publish/types.ts`, not
   here** — Status: accepted (2026-09-02, recorded as **ADR-092** because it
   spans this node, BP-045 and BP-058 and no one of them owns it)
   - Context: `tags` was declared in this surface and two `src/lib/publish/`
     nodes were specified to invalidate the tags "by name". Neither reading of
     "by name" is legal: an import is `src/lib → src/app`, and two string
     literals is two homes for one key.
   - Decision: the two builders move to BP-045's leaf and are re-exported here.
   - Alternatives considered: BP-005's `constants.ts` — rejected, a tag is a
     function of a row id, not a pinned value, and `tests/pins.test.ts` has
     nothing to assert about it; a `revalidate()` callback passed down from this
     surface — rejected, it inverts the direction at runtime instead of at
     compile time and leaves a job path with no caller to pass it.
   - Cost of reversing: one declaration and one re-export. The cost of not
     making it is a rule 6 violation that lands the day the first
     `revalidateTag` call is written, or two literals that stop matching.
