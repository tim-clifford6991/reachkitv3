...
/** **`OnPageFacts` is this node's — declared and owned here** (decision 5,
 *  2026-09-04), after WO-251 derived it against no blueprint at all. It is the
 *  counted facts of one fetched document — headings, question-shaped headings,
 *  direct-answer headings, the evidence-density inputs, schema types, Open
 *  Graph properties and the document's own access gates. **Counts only: no
 *  score, ratio, floor or band is a member**, which is the boundary that keeps
 *  the parse on this side of BP-024's arithmetic.
 *
 *  The member list is **not restated here** (rule 2.4, rule 2.5) — it is
 *  declared once, in the code this node governs, and cited by anchor:
 *  `src/lib/measure/parse.ts` › `export interface OnPageFacts`, whose own
 *  doc-comment traces every field to the `BUILD.md` §5 sentence it counts.
 *  BP-012's `StoredReport` (`onPage: OnPageFacts // BP-010`) and the six work
 *  orders that carry the same line are citing this declaration, not making
 *  their own. */
export interface OnPageFacts { /* src/lib/measure/parse.ts */ }
export function parseOnPage(a: { url: string; html: string }): OnPageFacts
  // deterministic and total: any string parses, an empty one yields every count
  // at 0, and it never throws (`## Error & edge behavior`, first bullet)
export function visibleText(html: string): string
  // the same extractor `readMeasuredText` below re-reads with — one text, so the
  // comparison set cannot disagree with the measurement (decision 4, rule 7.2)
...
5. **`OnPageFacts` is BP-010's, and its members stay in the code** —
   Status: accepted
   - Context: the type was named by `measureDomain`'s return signature here and
     by BP-012's `StoredReport`, copied verbatim into six work orders
     (WO-184, WO-185, WO-186, WO-189, WO-191, WO-192), and **declared by none of
     them**. WO-251 had to derive a shape to build the parse at all, flagged it
     as an open `rests-on` row, and its validator confirmed the gap and left the
     ownership call here (rule 7.1: one capability, one BP node, one module).
   - Decision: it is this node's. `src/lib/measure/parse.ts` falls inside this
     node's own `code:` globs and outside BP-024's, which names five files and
     not this one — so `structure.md` rule 2 already put the file here and the
     type follows the file. The `## Public interface` above **names** it,
     bounds it ("counts only"), and cites the declaration by anchor rather than
     transcribing sixteen members into prose.
   - Alternatives considered: **transcribe the members into this blueprint** —
     rejected under rules 2.4 and 2.5. That mints the second copy, and the two
     diverge on the first field added; the shipped declaration already carries
     each field's `BUILD.md` §5 provenance in its own doc-comment, so the copy
     would be strictly less informative than the anchor. **Home it on BP-024** —
     rejected: BP-024 is the verdict leaf, its arithmetic consumes counts and
     produces none, and its `code:` list does not include `parse.ts`. **Leave it
     undeclared** — rejected: a type six work orders name and no node owns is a
     rule-7.1 violation waiting for the second implementer to declare it again.
   - Consequences: a change to `OnPageFacts`'s members is a change under this
     node's `code:` globs and the console reports it against this node (rule
     5.6). WO-251's row is dischargeable and dispositioned. Cost of reversing:
     one paragraph — no data moves, since the facts are stored inside the
     `jsonb` report blob and no migration names these fields.
...
## Log
- 2026-09-04 amended — architect — W4 `rests-on` disposition (WO-251): `OnPageFacts` homed here — named and bounded in `## Public interface`, members cited by anchor to `src/lib/measure/parse.ts` rather than transcribed (rules 2.4, 2.5); decision 5 records the derivation. `parseOnPage` and `visibleText` named alongside it. No measurement, pin, `satisfies` or `covers` edge changed.
