...
rests-on:
  - claim: "The grounding passages REQ-050 criterion 1 keeps unchanged are stored in `drafts.grounded_fact` and nowhere else, so purging that column is the whole of what REQ-079 criterion 7 requires of them"
    ...
    reason: "Architect's wave-close read, 2026-09-04 (rule 2.3b), discharged against BP-014's approved decision 2, which is the only place the passage is written and says so: 'store the passage in `drafts.grounded_fact`, and treat that copy as customer page content for erasure (REQ-079 criterion 7 names it)' — with the alternative of storing a URL and re-fetching explicitly rejected, so no second copy and no re-derivation exists. BP-014's `## Data model delta` adds exactly one column for it. Purging that column is therefore the whole of what REQ-079 criterion 7 requires of grounding passages."
supersedes: []
code:
...
## Data model delta
`users` (additive):
- `deleted_at timestamptz null` — the tombstone. See **ADR-051**. **Created by
  BP-002's policy migration** (`*_rls*.sql`, WO-267), as the precondition of the
  `deleted_at is null` condition every request-scoped policy carries — this node's
  `*_users_erasure_*.sql` does not add it a second time. Everything else about the
  column is this node's: its meaning, the one place it is set (`deleteAccount`), the
  `purge_due_at` companion below and the purge.
- `purge_due_at timestamptz null` (`*_users_erasure_*.sql`) — `deleted_at + ERASURE_DAYS`, stored rather than
...
recorded that **REQ-079** was `status: in-review` with the owner's signature
pending on three folds carried into this node: the liveness scoping in criterion
4; the inversion of criterion 4's two WordPress populations under ADR-084; and
the rewriting of criterion 6's WordPress half into four sentences, one per
REQ-056 criterion 16 outcome, each with its own count. It read **REQ-056** and
**REQ-060** in the same state. **The owner has since approved all three, with
those three folds standing as they were written. The gate is now met and these
grounds hold.** No retroactive refusal is owed.

**What does not discharge with it:** `DeleteAccountResult.leftInWordPress` has
changed shape twice and the work order implementing this mail is still owed a
third re-extract. That is the planner's, stated here so it is not read as
covered. The two `## Open questions` below are owner-owed strings and are
untouched by the approval — approving the behaviour did not mint a sentence.
