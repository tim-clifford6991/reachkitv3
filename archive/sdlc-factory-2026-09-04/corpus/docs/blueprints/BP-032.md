...
rests-on:
  - claim: "Stripe delivers `checkout.session.completed` for a completed subscription Checkout Session at least once, and a session that never completed produces no such event — so the signature-verified webhook is a sufficient as well as a necessary provisioning trigger (BUILD.md §13, 'Webhook (signature-verified, the only provisioning path)')"
    disposition: open
    reason: "Left open deliberately, 2026-09-04 (rule 2.3b): Stripe's delivery guarantee for `checkout.session.completed` is settled by the first webhook received in test mode, not by a read of the corpus. Dischargeable, not yet discharged."
supersedes: []
code:
...
- `requestMagicLink` for an address with no account and a *completed* session held
  ...
  (REQ-020 c4). An address with neither answers `no_account`. Each answer is its
  own written line and says nothing beyond it — REQ-020 c4's three answers differ
  in words by design — and no timing or request-rate parity between the branches
  is built (decision 5).
...
5. **No post-submission parity between the three sign-in answers** — Status: accepted
   - Context: 2026-09-03, backward pass from REQ-098 round two
     (`requirements/history/REQ-098-099-2026-09-02-review-round-two-folded.md`).
     This node's edge behaviour had read a wording or timing difference between
     `requestMagicLink`'s branches as an account-existence oracle. Approved
     REQ-020 c4 requires the three answers to differ in words — one written line
     each — so whoever submits an address reads whether it has an account by
     design; the only silence promised is before an address is given (REQ-020
     c5), which is this function's single-input shape and BP-001's page. No
     blueprint owns a request limiter on this path — BP-061's one-live-token
     rule is a token budget, not a request limiter — so the parity clause was
     asserting sameness of a mechanism nobody had cut.
   - Decision: the branches are distinguished by their words and by nothing this
     node guards against — no constant-time padding, no per-branch or uniform
     request limiter, no test for either. The one-live-token rule stays BP-061's.
   - Alternatives considered: pad every branch to the slowest (the mail send) —
     rejected, it spends latency hiding a fact the answer states in writing. A
     request limiter inside this node — rejected, no criterion asks for one
     (REQ-098's non-goals declare abuse controls out), and if one is ever wanted
     it is a transport concern at BP-001's adapter, not a branch of this function.
   - Consequences / reversal cost: low — a limiter or timing guard is one wrapper
     at the adapter and one test, with no interface change and nothing persisted.
     That is why it is not pre-built. WO-127's file plan agrees as of 2026-09-03.
