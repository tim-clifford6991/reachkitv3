...
recorded that this node's 2026-09-01 edits — ADR-084's boolean split, the
WordPress delivery reaching `published` in the ordinary sense, the inverted
unpublish populations — restate what BP-045, BP-048, BP-049 and BP-058 specify,
and that those nodes derived from **REQ-056, REQ-060 and REQ-062** while all
three were `status: in-review`. It noted §3's gate is literally silent on a node
with no `satisfies` edge, and refused to shelter behind that. **The owner has
since approved all three, REQ-060 criterion 1's inversion included. What this
node says about WordPress is now true on the approved text, not conditional on
a signature.** The leaves' own addenda are discharged on the same grounds.
