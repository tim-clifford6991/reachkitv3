...
blocked-by: []
rests-on:
  - claim: "A corrected free report ordinarily fits inside CAPS.FREE_C 12c: the pair is 11.1c + 0.2c per SERP on which Google served an asynchronous AI Overview (ADR-094 decisions 5 and 3a), so it fits whenever at most four of the twelve did, and cache hits on the scan widen that. Two facts underneath it are unmeasured and only live traffic can settle them: how often Google serves an asynchronous overview on these twelve queries (the vendor says only that it does so 'frequently'), and BP-008's 30-day cache hit rate on the SERPs and suggestions call. If both run against us the correction truncates part-way through its SERP fan-out and REQ-094 criterion 7's partial result is the ordinary outcome for corrected reports. Raised 2026-09-03 with decision 4's amendment; restated the same day after decision 3a."
    disposition: undischargeable
    reason: "Architect's wave-close read, 2026-09-04 (rule 2.3b). The row states its own impossibility in its own words: 'Two facts underneath it are unmeasured and only live traffic can settle them' — how often Google serves an asynchronous AI Overview on these twelve queries, and BP-008's 30-day cache hit rate. Both are frequencies over real traffic, not properties of anything this factory builds. Recorded rather than cleared: if both run against us, REQ-094 criterion 7's partial result becomes the ordinary outcome for corrected reports, which is a customer-visible consequence — reported to the owner at this wave close as a risk carried, not as a question asked."
supersedes: []
code:
...
- A correction's own cost and clock are BP-012's: it belongs to the scan it
  corrects, spends no further allowance, and re-runs steps 2–5 against a
  corrected category. Corrected 2026-09-03 (decision 4, then ruled — decision
  4a): **the scan reserves ~9.0¢** (0.6¢ two nano calls + 1.8¢ suggestions +
  4.8¢ twelve live SERPs at ADR-094's surcharge rate + 1.8¢
  `ranked_keywords`@50) and **settles at 6.6¢ + 0.2¢ per SERP on which Google
  actually served an asynchronous overview** (ADR-094 decision 3a: the ledger
  records the vendor's documented charge, the reservation is only what the cap
  is checked against). **The correction is ~4.5¢** (1.8¢ suggestions + 0.3¢ one
  nano + 2.4¢ twelve live SERPs, **un**flagged since ADR-094 decision 5 — its
  twelve queries are new and none can be a cache hit). So the pair is
  **11.1¢ + 0.2¢ × a against `CAPS.FREE_C` 12¢**, where `a` is how many of the
  twelve carried an asynchronous overview, both spenders inside one
  `CostContext`: **it fits whenever a ≤ 4**, and reaches 13.5¢ only where Google
  served one on all twelve. Cache hits on the scan widen that further. Before
  decision 3a the ledger read 9.0¢ on every uncached scan whatever `a` was, and
  the correction truncated every time; the residual now is the genuinely
  expensive scan, not the accounting. Where it still does not fit, the
  correction degrades part-way through its SERP fan-out by the rule below.
...
4. **The correction shares the scan's one ceiling and degrades exactly as the
   scan does; the ceiling's value is not this node's** — Status: accepted
   (mechanics: parameter, rule 1.1; cap value: owner's, `OWNER-QUESTIONS.md`
   item 9)
   - Context: ADR-094 priced one scan's own worst case (~7.2¢ for BP-025's
     spend) and did not price the second spender REQ-094 gives every free
     report — a correction, which criterion 3 requires to spend "no further
     scan allowance" and open "no second spend ceiling". Taking the corpus's
     current figures (BP-025 decision 2's two-nano-call reading, ADR-094's
     surcharge rate, BP-005's corrected price book): the scan is ~9.0¢ worst
     case and the correction, whose twelve queries are all fresh, ~6.9¢ —
     15.9¢ against a 12¢ cap, or 11.1¢ even with no surcharge charged at all.
     Neither this node nor BP-023 had stated which `CostContext` a correction
     runs inside, nor what "opens no second spend ceiling" means mechanically.
   - Decision, mechanics only:
     - **Composition:** a correction runs inside the *same* `CostContext` the
       scan it corrects opened — BP-023's `## Module / boundary` already
       states "it never calls `claimFreeScanSlot` and opens no second cost
       context"; this decision is that no second `CAPS.FREE_C` ceiling exists
       to compose with the first, satisfying criterion 3 by construction
       rather than by a second check.
     - **Degradation order:** unchanged from BP-007 decision 1. The
       correction's re-derivation runs `CORRECTION_SCOPE.reDerive`'s order
       (market → questions → ai_answers → google_presence → rivals → score);
       a cap hit mid-sequence degrades exactly as an initial scan's does —
       whatever was bought stands, the rest is `unmeasured / not_attempted`,
       and `nextCorrectionState`'s `produced_no_report` arm (decision 2) is
       what "the correction did not complete" (criterion 7) already means.
       No new state, no new field: the machine specified above already
       represents this outcome.
     - **What the corrected pass does mechanically:** nothing different from
       an initial scan's own steps 2–5, run again with the corrected category
       as their input. No new function, no new interface.
   - **What was not decided here, and now is (2026-09-03):** whether
     `CAPS.FREE_C` should move, whether the second pass should decline the
     surcharge flag, or whether degrading is the accepted outcome. All three
     changed a number a customer reads, so all three were the owner's; the
     ruling is decision 4a below.
   - Alternatives considered: a second, smaller `CAPS` entry for the
     correction alone — rejected, it is exactly the "second spend ceiling"
     criterion 3 forbids; degrade the correction by a different rule than the
     scan's (e.g. refuse outright rather than partially complete) — rejected,
     it would need its own ceiling-and-refusal machinery for one code path
     that already has one.
   - Consequences: no interface or schema change. The NFR budget above states
     the corrected arithmetic; whichever lever item 9 chooses, the mechanics
     recorded here do not move.
   - Cost of reversing: none stored — this decision states an order that
     already existed in BP-023 and BP-007's own machinery; it does not
     introduce new state to reverse.

4a. **The cap stays at 12¢ and the correction stops paying for asynchronous AI
   Overviews** — Status: accepted (owner ruling, 2026-09-03, closing decision
   4's open half)
   - Context: decision 4 fixed the mechanics and left three levers to the
     owner (`OWNER-QUESTIONS.md` item 9), because each changed a number a
     customer reads. The owner's words on the cap, verbatim: "12c is already
     at the top end of what im willing to spend therefore would keep the limit
     here for the current time. this is a lead magnet but I am not yet
     confident what a user wants from us therefore wasting money on it is a
     crime."
   - Decision: lever 2. `CAPS.FREE_C` stays 12¢ (BP-005 holds the pin and its
     annotation), and the correction's twelve SERPs decline the async-AIO
     surcharge — **ADR-094 decision 5**, which owns that call-site rule; this
     entry owns only what it does to the ceiling this node composes.
   - Consequences, stated as arithmetic rather than as intent:
     - the correction's ledgered spend falls 6.9¢ → 4.5¢ and the pair 15.9¢ →
       13.5¢ against 12¢ (NFR budget above);
     - a complete correction is reachable whenever the corrected scan ledgered
       ≤ 7.5¢, and not otherwise. Under the pre-ruling figures no fresh scan
       left enough for a flagged correction to buy more than about two of its
       twelve SERPs. The lever alone narrowed that but did not remove it — the
       ledger read 9.0¢ on every uncached scan — so **ADR-094 decision 3a was
       taken in the same pass** (rule 1.1, a ledgering convention with no
       customer-visible number in it): the scan now settles at 6.6¢ + 0.2¢ × a,
       and the correction completes whenever Google served asynchronous
       overviews on at most four of the twelve. What remains is a scan that
       genuinely cost the money, plus the cache-hit assumption the front
       matter's `rests-on` row carries;
     - a correction that is truncated still **produced a report**, so
       `nextCorrectionState` moves it to `used`, not to `failed_once`
       (decision 2, unchanged) — the reader gets a corrected report stating
       what it could not measure, and no second attempt is minted by a partial
       result;
     - the corrected card counts only the AI answers Google had cached and
       says so. The disclosure is a **customer-visible string and therefore
       not this node's**: REQ-094 carries the criterion, BP-019's registry the
       words, and BP-025's `AiAnswersCard` the state they render from.
   - Alternatives considered: the other two levers — raise the cap to ~18¢, or
     change nothing and let a flagged correction stop after two SERPs. Both
     were the owner's to weigh and both were declined; ADR-094's alternatives
     section records them beside the call-site rule they turn on, and they are
     not re-argued here (rule 2.4).
   - Cost of reversing: one boolean at one call site (ADR-094's to state) and
     one pin. Nothing is stored differently, which is why the reversal is
     cheap and its symptom — a correction that quietly measures four searches
     instead of twelve — is not loud.
...
## Log
- 2026-09-03 reviewed — research/efficiency — cost-envelope audit: one REVIEW line raised (the correction's cost envelope predates ADR-094 and the pair exceeds `CAPS.FREE_C`). No decision, criterion or pin changed.
- 2026-09-03 amended — architect — settled as far as parameter authority allows (owner ruling: "settle it as far as it is a parameter"). Decision 4 added: the correction shares the scan's one `CostContext` (no second ceiling), degrades by BP-007 decision 1's existing rule, and the corrected arithmetic (9.0¢ scan + 6.9¢ correction against 12¢) is stated in the NFR budget. The cap value itself is not moved — `OWNER-QUESTIONS.md` item 9, unchanged. `satisfies` unaffected; no requirement text changed.
- 2026-09-03 amended — architect — `/decide`, owner ruling on `OWNER-QUESTIONS.md` item 9 (lever 2): decision 4a added — `CAPS.FREE_C` stays 12¢ and the correction declines the async-AIO surcharge (call-site rule at ADR-094 decision 5); NFR budget re-costed (correction 6.9¢ → 4.5¢, pair 15.9¢ → 13.5¢ against 12¢) and one `rests-on` row raised (`open`) for the cache headroom a complete correction needs. No interface, state machine, pin or requirement text changed; `satisfies: [REQ-094]` unaffected.
- 2026-09-03 amended — architect — `/decide` follow-up: NFR budget re-costed against **ADR-094 decision 3a** (the scan settles at 6.6¢ + 0.2¢ × a instead of ledgering 9.0¢ always), so the pair is 11.1¢ + 0.2¢ × a and a complete correction fits whenever at most four of the twelve carried an asynchronous overview; decision 4a's consequence updated and the `rests-on` row restated over the two unmeasured vendor facts it now turns on. No interface, state machine, pin or requirement text changed.
