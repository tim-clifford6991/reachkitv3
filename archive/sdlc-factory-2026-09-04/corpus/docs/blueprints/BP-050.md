...
## NFR budget
- Weekly tier: standard queue, **≈8.7¢ ledgered per site** (corrected
  2026-09-03: `AI_MODE_STD_C` was pinned at half the vendor's price — BP-005
  decision 3 addendum; the weekly battery moves from 2.2¢ to 2.88¢, still far
  inside `CAPS.WEEKLY_C` 40¢) (BP-012's ceiling), fanned out at BP-003's fixed
  concurrency so one slow site never starves the week. Its target SERPs pass
  `CACHE_WINDOWS_D.serpWeeklyRecheck` (7d) through `serpOrganic`, not the
  pinned `serp` (30d) — BP-008 decision 4 closes the freshness-window
  exception `BUILD.md` §6.4 names and this node's re-check depends on.
...
## Log
- 2026-09-03 reviewed — research/efficiency — cost-envelope audit: two REVIEW lines raised (the AI Mode price under-count; no home for `BUILD.md` §6.4's weekly-recheck freshness exception).
- 2026-09-03 amended — architect — settled both: NFR budget corrected to ≈8.7¢/site (BP-005's price correction), and this node's target SERPs now pass BP-005's new `CACHE_WINDOWS_D.serpWeeklyRecheck` (BP-008 decision 4) instead of the pinned 30-day `serp` window. `satisfies` unaffected; no requirement text changed.
