...
(`scans.domain`, `report.market.profile.category`, `report.rivals`), owned by
BP-012 — the category is a field of BP-025's `Profile` and stays there rather
than gaining a second home at the top of the section (**ADR-095** decision 4).
...
3. **A value carrying no goal shows its own measurement's counts and their
   date; a value carrying one shows the goal** — Status: accepted, re-derived
   2026-09-01
   - Context: this decision was originally written against an open
     `REVIEW(untestable)` line on REQ-071's goal-carrying limb — the objection
     being that "each affected number carries its goal in place of a change"
     cited REQ-041 criterion 4, which gave a goal only to a headline number, so a
     rival distance or a charted point had no pass condition. **That line no
     longer exists** and the gap it named is closed at the requirement, not
     worked around here: criteria 12 and 13 were narrowed into two parts on
     2026-09-01, and REQ-041 criterion 11 was amended to match.
   - What the requirement now states, and this node builds to:
     - Every affected number, distance and point is shown with **no change
       figure** across the break — criteria 12 and 13's first part, testable for
       everything they cover.
     - A number that carries a goal (REQ-041 criterion 4) carries **the goal** in
       place of the change. Criterion 4 as folded also settles which numbers
       those are: the four headline numbers, and no others — "the gap module
       leads with no headline number".
     - A value carrying **no** goal, which would otherwise have been shown
       together with a previous value from the far side of the break, "is shown
       instead with the counts from its own measurement and the date they were
       taken, never with that previous value" (criterion 12, and criterion 13 in
       the same terms for a domain change).
   - Decision: `changeMarkers` supplies the dates and nothing else. The third
     bullet is a rendering obligation on the surfaces that hold those values, and
     its one typed home is BP-038's `PreviousRatio.no_previous_ratio`, whose
     `because: 'series_break'` arm carries exactly those counts and that date.
     This node states the dates; BP-038 states the shape; neither restates the
     other (rule 2.4).
   - What is withdrawn: the sentence "Nothing is invented to stand in for a goal
     that REQ-041 does not define" was true of the corpus as it stood and is now
     misleading. Nothing is invented still — but the reason has changed. It is no
     longer that the requirement leaves the case undefined; it is that the
     requirement **defines** it, as the value's own counts and their date, and
     that is a smaller, more testable thing than a goal.
   - Alternatives considered: minting a goal per rival distance and per series
     point — still rejected, and now by the requirement's own words as well as by
     rule 1.2: criterion 4 says such values "carry no goal, and render without
     one"; carrying the previous ratio across the break with a change figure
     suppressed — rejected, criterion 12 forbids showing it "together with a
     previous value measured under the old question set" at all, not merely
     forbidding the arithmetic on it.
   - Consequences: no open question survives here. A surface that shows a
     goal-less value across a break and cannot produce its own measurement's
     counts is a defect in that surface, not a gap in the requirement.
   - Cost of reversing: low; one union arm in BP-038 and one function's return
     shape here.
...
from **REQ-071** while that requirement was `status: approved` — the moment §3's
blueprint gate was met and read — as were the five requirements it depends on
(REQ-021, REQ-043, REQ-063, REQ-065, REQ-070). It satisfies exactly that
requirement, lives inside BP-011's boundary, and overlaps no sibling's globs.

REQ-071 is `status: in-review` from 2026-09-01, sent back through the owner gate
by the review round that narrowed criteria 12 and 13. That narrowing is folded
into decision 3 above — "the promise is narrowed, not withdrawn: nothing that
was forbidden before is permitted now"
(`requirements/history/REQ-029-041-060-064-071-2026-09-01-review-lines-closed.md`)
— so no interface in this node changes and the approval stands on the current
text. The dependency that made criteria 12 and 13 untestable — REQ-041 defining
"a number that carries a goal" only for a headline number — is settled by
criterion 4's own fold, which is why REQ-071 carries no `blocked-by` on REQ-041
and why this node needs none either. If the owner's approval changes either
criterion rather than ratifying it, this status reverts by the librarian's
retroactive refusal (rule 3.2).

## Open questions
None. The one that stood here was REQ-071's `REVIEW(untestable)` line on criteria
12 and 13; it was folded into the criteria and deleted on 2026-09-01, and
decision 3 is re-derived against the text that replaced it.
