...
describe("the five keys the owner ruled 2026-09-11 (DECISIONS 2026-09-11, #516)", () => {
  ...
  it("the only owed keys are §7's two unreachable arms, and the unwritten ones are the ledger's named set", () => {
    expect([...OWNER_OWED].sort()).toEqual([...WITHDRAWN].sort());

    // `AWAITING_COPY` is checked against the ledger rather than against
    // nothing (#322): the snapshot records that set by name, per partition,
    // so a key that quietly stopped being written shows up as a diff
    // naming it.
