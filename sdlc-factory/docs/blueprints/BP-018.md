...
4. **Standing law 2 decides whether a mark is drawn; `BUILD.md` §2.4 decides
   how a drawn mark is labelled** — Status: accepted (parameter, rule 1.1)
   - Context: §2.4's "every bar and point is direct-labelled (name + value)"
     and `00-project.md` standing law 2 ("a number that answers no customer
     question is not rendered, even if we hold it") both bind every chart this
     module renders, and neither artifact stated which governs which
     question. Read as one rule about which measurements must be drawn, §2.4
     licenses exactly what law 2 refuses — the free report's three driver
     mini-bars (BP-024), the AI matrix's per-row fraction (BP-025) and the
     rival-gap module's redundant encodings (BP-038), all recorded
     screen-by-screen in `design/meaning-over-data.md`.
   - Decision: law 2 decides **whether** a measurement reaches a surface as a
     mark at all; §2.4 decides **how** a mark that is drawn is labelled. So
     §2.4 reads "every mark that is rendered carries its name and its value,"
     never "every measurement is rendered as a mark." This node's charts hold
     the labelling half only — the drawing decision belongs to the surface
     node rendering the measurement, argued there against law 2.
   - Alternatives considered: read §2.4 as also requiring every measurement be
     drawn — rejected, that reading is what produced the three findings above;
     leave the two rules unordered and let each surface resolve the conflict
     itself — rejected, a standing law binds every artifact under it, and
     re-deriving the order at each of thirteen surface nodes is the
     N-registries failure `structure.md` rule 5 exists to prevent.
   - Consequences: no interface change here. A surface node that drops a mark
     under law 2 (BP-024, BP-025, BP-038) cites this order instead of
     re-arguing it.
   - Cost of reversing: none stored — an interpretive order, not a schema or
     interface. Reversing re-opens the ambiguity this decision exists to
     close.
...
## Log
- 2026-09-03 amended — architect — settled the `REVIEW(conflict with PROJECT)` line raised by `research/ux-simplification`: added decision 4, stating the order between `BUILD.md` §2.4 and `00-project.md` standing law 2 (law 2 decides whether a mark is drawn; §2.4 decides how a drawn mark is labelled). Parameter under rule 1.1 — no customer-visible consequence, so no escalation. No interface or status change.
