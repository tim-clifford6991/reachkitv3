...
- Free path spend, **initial pass**: `keyword_suggestions` 1.8¢ + 12 live SERPs,
  `loadAsyncAiOverview: true` since ADR-094 — 2.4¢ of actual vendor charge where
  Google served no asynchronous overview to fetch, 4.8¢ where it did and 4.8¢
  *ledgered* either way (ADR-094 decision 3) — + **two** nano calls ≈ 0.6¢ (see
  Decisions 2): **7.2¢ ledgered** for this node's own spend, inside `CAPS.FREE_C`
  12¢ together with F4's 1.8¢ (bought by BP-010, not this node).
