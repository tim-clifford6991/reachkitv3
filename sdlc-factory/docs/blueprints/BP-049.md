...
meet.** The edits of 2026-09-01 — the three-outcome union, the identity split,
the site condition, and ADR-084's arrival at this node — derive from **REQ-062,
`status: in-review`**, and read **REQ-063**, **REQ-060** and **REQ-043**
(`in-review`, all three) for their consequences. All four carry **zero open
...
under rule 3.2 on the design, not on the gate, and this node does not claim
otherwise. If the owner's signature changes what any of those criteria promise
rather than ratifying them, the librarian reverts this status retroactively and
names the clause. No `blocked-by` is added: an `in-review` requirement mid-gate
is not a conflict with another artifact (rule 2.3), and the two `rests-on` rows
above are the honest record of what is unverified.
