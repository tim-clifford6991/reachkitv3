...
- [ ] REVIEW(conflict with BP-005): "≈8¢ ledgered per site" rests on
      `DATA-COSTS.md` §3's weekly roll-up, whose battery line uses
      `AI_MODE_STD_C: 0.06`. `registry/evidence/RESEARCH-dataforseo-endpoints.md`
      §3 quotes the vendor's page at "$0.0012" — 0.12¢ — so the weekly battery
      is 2.88¢ rather than 2.2¢ and the weekly refresh ≈8.7¢. It stays far
      inside `CAPS.WEEKLY_C` 40¢ and changes no customer-visible number; it is
      the same one-line correction BP-005 carries.
- [ ] REVIEW(gap): this node re-measures every active site once a week and
      states no freshness window for anything it re-buys. `BUILD.md` §6.4 pins
      the SERP cache at 30 days **"except the weekly target re-check"**, and
      that exception has no home in the corpus — BP-005 pins a single
      `CACHE_WINDOWS_D.serp: 30` and BP-008 declares no endpoint-to-window map
      (see its line). If the pinned 30 is what `serpOrganic` passes, this job's
      thirteen target SERPs are cache hits in three weeks of every four: the run
      completes, `WeeklyOutcome` reports `status: 'done'`, `accountForWeek`
      returns `{ kind: 'complete' }` with this week's `measuredAt`, and the
      values under that date are up to a month old — which is REQ-065's weekly
      promise and REQ-041 criterion 7's "no earlier week's value labelled with
      that week's date", both broken silently and both by a cache hit rather
      than by a failure anything reports. This node is where the exception has
      to land, because it is the only caller that knows a re-check is what it is
      doing.
