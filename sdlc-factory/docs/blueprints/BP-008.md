...
rests-on:
  - claim: "Labs `ranked_keywords` on a root domain returns rows for pages served on the `content.` subdomain; if it does not, hosted pages' wins are invisible to the growth chart and the client must query with subdomain inclusion or track the subdomain as a second target (BUILD §16 milestone 2 is the first measurement)"
    disposition: open
  - claim: "DataForSEO refunds the `load_async_ai_overview` surcharge to the account balance after the fact and never adjusts the amount this client observes at call time, so ledgering the surcharge-inclusive price on every flagged call cannot under-count spend (ADR-094)"
...
- **`load_async_ai_overview` is the never-list's one admitted exception**
  ...
  only BP-025's twelve-question free-matrix call passes `true`. Every other
...
- 12 live SERPs for the free battery complete within the 60-second promise at
  ...
  (`BUILD.md` §6.2).
- Free battery ≤ 6.3¢ target, 12¢ cap; weekly paid battery 2.2¢. **Since
  ADR-094**, the free matrix's SERPs carry `loadAsyncAiOverview: true` and the
  worst case rises to ~7.2¢ for BP-025's own spend (see that node's NFR
  budget) — still inside the 12¢ cap. The paid weekly battery is unchanged.
...
- [ ] REVIEW(gap): this node is the only caller that buys vendor data through
      BP-007, whose `recordFetch` makes **`freshnessDays` a required per-call
      argument**, and no function on this interface takes one and no line in
      this file says which window each endpoint passes. BP-005 pins
      `CACHE_WINDOWS_D: { own: 7; rival: 30; serp: 30; suggestions: 30 }` and
      only BP-052 reads it. The consequence is not cosmetic: `BUILD.md` §6.4
      states the SERP window with an exception — *"SERPs 30d (except the weekly
      target re-check)"* — and **no artifact in the corpus carries that
      exception**. If `serpOrganic` passes the pinned 30, BP-050's weekly
      re-measurement serves cached SERPs in three weeks of every four, and
      REQ-065's promise that every active site is re-measured *once a week* is
      met by a monthly measurement wearing a weekly date. State the
      endpoint-to-window map here, and give the re-check exception a home.
- [ ] REVIEW(gap): BP-007 keys the cache `source + cacheKey + policyVersion`
      and **no node states what `cacheKey` is for any endpoint**. For
      `serpOrganic` the choice decides the product's largest recurring cost: a
      key of query + `SERP_LOCATION` is shared across every customer whose
      market contains that search, so a market's thirteen weekly target SERPs
      are bought once however many customers track it; a key carrying the site
      id buys them once per customer. BP-050's NFR budget ("≈8¢ ledgered per
      site") and `DATA-COSTS.md` §5's COGS roll-up both assume the second and
      neither says so, and nothing in the corpus has priced the first. A cache
      key is a parameter (rule 1.1) — this node's to choose and to state, not
      to leave to the first call site that writes one.
- [ ] REVIEW(conflict with BP-005): the NFR budget's "weekly paid battery 2.2¢"
      is computed from `AI_MODE_STD_C: 0.06`, which
      `registry/evidence/RESEARCH-dataforseo-endpoints.md` §3 refutes against
      the vendor's own page (AI Mode standard is "$0.0012" — 0.12¢). At the
      vendor's price the battery is 12 × (0.12 + 0.12) = **2.88¢**. The same
      line's "Free battery ≤ 6.3¢ target" is superseded twice over: BP-011's
      NFR budget corrects the free-scan headline to **6.6¢** on the two-nano-call
      reading, and ADR-094 takes its worst case to **9.0¢** (BP-025's 7.2¢ plus
      F4's 1.8¢). Three figures, one edit each; none reaches a cap.
