...
**Status grounds addendum, 2026-09-01 (rule 3.2).** Nothing in this node's own
interface changed on 2026-09-01; the two bullets under *Error & edge behavior*
record what the rulings did to a kind it registers and to a test it owns. Both
read **REQ-062** (`status: in-review`, zero open `REVIEW(...)` lines after two
review rounds), so §3's gate is not met for them either; they are assertions
under rule 3.2 and are statements of fact about the register rather than design
changes. REQ-064's own criterion text is unchanged, as the paragraph above
records.
