...
rests-on:
  - claim: "Ranking data for the customer's root domain includes rows for pages served on the hosted content. subdomain, so pages ReachKit publishes register as the customer's own wins on the growth chart (inherited from REQ-041)"
    ...
    reason: "Architect's wave-close read, 2026-09-04 (rule 2.3b). Inherited from REQ-041 row 1 — the row says so — and the home is there, still `open` and dischargeable by one live vendor call (rule 2.4). No observation at this node settles it independently."
  - claim: "Ten ranked searches is the point below which a rival-to-customer ratio misleads more than it informs (inherited from REQ-041; BUILD.md 6.6 pins the threshold, nothing measures it)"
    disposition: undischargeable
    reason: "Architect's wave-close read, 2026-09-04 (rule 2.3b). Inherited from REQ-041 row 2 — the row says so — and the home is there, dispositioned undischargeable in this same pass: what a ratio 'misleads' a reader into is not observable here. The threshold BUILD.md 6.6 pins stands unchanged."
  - claim: "The last n weeks' `report.answers` (BP-025's per-question AI-citation cells) are readable from the same weekly `scans` rows this node's NFR budget already reads as 'the last 12 weekly report headers', so windowing the ai_answers headline (decision 5) needs no wider read than that budget already declares. If 'headers' excludes the answers blob, the read must widen and the NFR budget line needs updating."
    ...
    reason: "Architect's wave-close read, 2026-09-04 (rule 2.3b), discharged against BP-012's declared interface. There is no narrower read than the one this node's NFR budget already declares: `scans.report` is a single `jsonb` column holding one `StoredReport`, and `answers: AiAnswersCard` is a member of it, so a read that returns 'the last 12 weekly report headers' returns the answers blob with them — a header read that excluded a member of the same value would have to be a second, projecting query, and BP-012 declares none. The NFR budget line needs no widening."
supersedes: []
code:
...
/** **Three keys, not four, since 2026-09-03**: `score` was retired with the
 *  composite score's tile (decision 7, owner ruling `OWNER-QUESTIONS.md` item
 *  8). */
 ...
 *  written in this module. All three carry a value — `pages_published` was ruled
 *  by the owner on 2026-08-31 at **30**, a month of daily pages, and
 *  `ai_answers` is **4 weeks of the trailing window** since the owner's item-7
 *  ruling (decision 5a) — so `value` is not optional and no tile renders a
 *  goalless headline. */
 ...
 *  the windowed reading that replaced it, and it is the only place movement on
 *  this card is shown — at the aggregate, never broken out by question or by
 *  single week.
 *
 *  **Since the owner's ruling of 2026-09-03 (`OWNER-QUESTIONS.md` item 7,
 *  decision 5a) this is not a context value beside a headline — it *is* the
 *  headline.** The tile shows one reading: the count of weeks the customer was
 *  present, named in the window it was counted over. The level out of twelve
 *  tracked questions is not rendered here at all. */
...
export interface OverviewModel {
  ...
  // No `score` module: the composite score has no tile on Overview (decision 7,
  // owner ruling 2026-09-03) — not read, not passed, no slot to render into. It
  // remains the free report's verdict, which is BP-024's.
  /** One reading, one number (decision 5a). The headline value is the windowed
   *  presence count with the window it was counted over — `k of n weeks` — and
   *  `headline.delta` is never populated (decision 5). `context` is empty: the
   *  level out of twelve tracked questions is not shown beside it. */
  aiAnswers: Module<AiPresenceWindow>
...
- **The AI tile shows one reading and one denominator** (decision 5a, owner
  ruling 2026-09-03): `citedWeeks` of `windowWeeks`, the window named on the
  tile. `aiAnswers.headline.delta` is never populated (decision 5) and
  `context` is empty, so nothing on the tile is read against twelve tracked
  questions. The goal that renders beside it is `GOAL_VALUES.ai_answers = 4`
  — every week of the trailing window — which is in the headline's own unit,
  satisfying criterion 4 without a second denominator.
- **Where `windowWeeks < AI_PRESENCE_WINDOW_WEEKS`** — a change marker inside
  the last four weeks truncates the window — the tile states the shorter window
  it actually counted and the goal does **not** rescale: criterion 4 fixes one
  goal for every customer, and a goal that moved with each customer's last
  market change would be a per-customer number the product never promised. Until
  the window refills, the goal is stated as unreached, which is what it is.
...
- **No text in this screen renders below the type floor at any band**
  ...
  renders.
- **Two stat tiles, not three** (decision 7): AI presence and pages published,
  beside the growth chart. They collapse by ADR-093 decision 5's rule; how two
  tiles sit at `wide` where three used to is layout, which is design-guardian's
  under rule 7.3 and is not decided here (the same routing decision 6 uses). The
  "max one headline number per module" rule is unchanged by band.
...
1. **"Headline number" is defined by the `headline` slot, not by a list** — Status: accepted
   ...
   - Decision: a headline number is the value passed to a module's `headline`
     ...
     such values, carry no goal, and render without one." Three modules lead with
     a number, so there are three headline numbers and three goals — four until
     the composite score's tile was removed on 2026-09-03 (decision 7).
...
2. **Three of the four goals are derived from pinned specification values; the
   ...
   - Decision, with derivation:
     ...
     - `ai_answers = 6` — `BUILD.md` §4.5, the AI-answers tile: "dashed goal dots
       + *goal: 6*", out of the twelve tracked questions. **Superseded
       2026-09-03 by decision 5a**: the tile now shows one reading in weeks of
       the trailing window, and the goal is 4 in that unit. The derivation above
       is kept because it is what the pin meant while it stood, not because it
       still holds.
     - `score = 50` — `BUILD.md` §5 pins the band boundaries "0–24 Invisible ·
       ...
       findable, so what reaching it means is already written. **Retired
       2026-09-03 by decision 7**: the composite score has no tile on Overview,
       so this goal has no headline to carry. BP-005 no longer declares it.
...
5. **The `ai_answers` headline never shows a raw week-over-week delta; a
   ...
   - Alternatives considered: per-question weekly movement (a delta on each of
     ...
     node.
   - Consequences: **as written, this decision left both readings on the tile**
     — the twelve-question level with its goal, and the windowed count beside
     it — and said they were not to be conflated. That is what decision 5a
     ends; the windowed reading survives and is the headline. Nothing else in
     this decision moves: the delta is still never populated and the window is
     still truncated at BP-056's most recent change marker.
...
5a. **One reading on the AI tile, and the goal re-denominated to 4 weeks** —
   Status: accepted (amends decision 5's consequence; owner ruling 2026-09-03;
   the value is a parameter, rule 1.1)
   - Context: decision 5 kept both readings and stated outright that they "are
     not to be conflated into one number" — a tile asking a founder to hold two
     meanings of "AI answers" apart, across three denominators (a level out of
     twelve tracked questions, a goal of 6 out of twelve, and `citedWeeks` out
     of four). A design that has to instruct the reader not to conflate two
     numbers has rendered one too many, which is `00-project.md` standing law
     2's zero-knowledge limb. Which reading survives is a number a customer
     reads, so it was the owner's: `OWNER-QUESTIONS.md` item 7, ruled as
     audited — one reading, the count of weeks the customer was present, named
     in the trailing window.
   - Decision: `aiAnswers` is a `Module<AiPresenceWindow>`; its headline is
     `citedWeeks` of `windowWeeks` and its `context` is empty. The level out of
     twelve tracked questions is not rendered on this screen at all.
     `GOAL_VALUES.ai_answers` moves **6 → 4** and changes unit with it.
   - Derivation of 4 (rule 1.1, this node's to choose): the goal has to be in
     the surviving reading's unit or the tile carries two denominators again,
     which is the defect the ruling ends. In weeks of the trailing window the
     reachable range is 0…`AI_PRESENCE_WINDOW_WEEKS`, pinned at 4 by decision 5.
     Of those, 4 is the only value that states a meaning without inventing a
     threshold — named in an AI answer in **every** week of the window, which is
     exactly the "presence across several weeks, not the difference between two
     weeks" that `RESEARCH-ai-answer-stability.md` §6 gives as the honest unit.
     1, 2 or 3 would each be a partial-credit line nobody stated, and would have
     to be defended as a level of acceptable absence. 6 cannot survive at all:
     it exceeds the window's own maximum, so the tile would render "4 of 4
     weeks · goal 6".
   - **Before restoring `goal: 6` from `BUILD.md` §4.5**, read this entry: §4.5
     still states "AI answers `n/12` … dashed goal dots + *goal: 6*" in writing,
     and it is no longer what this tile renders. The divergence is recorded here
     rather than by editing §4.5, the same treatment ADR-094 gives §6.2.
   - Alternatives considered: keep the twelve-question level as the headline and
     the window as context — the mirror ruling, not what the owner took, and it
     keeps the noisier of the two readings decision 5's own evidence rejects;
     drop the goal from this tile entirely — rejected, criterion 4 requires
     every headline to carry a goal and a single tile exempt from it is a second
     rule; rescale the goal to each customer's own truncated window — rejected,
     criterion 4 fixes one goal for every customer.
   - Consequences: BP-005's pin moves and its unit changes (that file holds the
     value, this one derives it — rule 2.4). What reaching the goal *means* is a
     customer-visible string, `GOALS.ai_answers.meansKey`, and the owner's;
     REQ-041 criteria 4 and 12 are the analyst's half of this ruling.
   - Cost of reversing: one pin, one type parameter and the tile's template. The
     trust cost is the two-denominator tile the ruling removed.

6. **The rival-gap row's redundant encoding is this node's finding; which of
   the three is dropped is design-guardian's** — Status: accepted
   - Context: `RivalGapModule` renders, per rival, a series, an endpoint
     distance and a previous-value badge, and `BUILD.md` §4.5 adds one
     written line explaining how to read the series — evidence the series
     does not read on its own. Three encodings of one comparison for the one
     question REQ-041 criterion 8 promises is more marks than the promise
     needs, which is `00-project.md` standing law 2's concern; which mark is
     registered chart geometry is rule 7.3's — the design system's, not this
     node's or the architect's (constitution §4's agent table: design-guardian
     decides tokens, components, layout).
   - Decision: this node records the redundancy and stops there. No encoding
     is removed from `RivalGapModule`'s type by this pass — the type carries
     `series`, `ranked`/`ratio` and `previous`/`series` today and continues to
     until a preview pass (rule 7.3) on this module settles which one goes.
     Not an owner question: no number a customer reads changes by dropping a
     redundant mark, only which mark carries it.
   - Alternatives considered: pick one here — rejected, it is a layout and
     chart-geometry call the constitution assigns to design-guardian, not the
     architect; leave the finding as an open `REVIEW(...)` line indefinitely —
     rejected, the finding is settled (a redundancy exists and whose call it
     is), only the specific choice is outstanding, and that belongs on the
     work this module's next `ui: yes` work order carries, not on this
     blueprint's open questions.
   - Consequences: none for this interface today. The planner's file plan for
     any work order touching this row should route the specific choice
     through `/design`'s preview gate rather than assume one.
   - Cost of reversing: none — nothing is removed yet.

7. **The composite score has no tile on Overview** — Status: accepted (owner
   ruling, 2026-09-03)
   - Context: `OWNER-QUESTIONS.md` item 8. The score tile sat beside
     searches-appeared-in, AI presence and pages published — three measures a
     ...
     in words and the concrete tiles answer it in measurements, and JN-005's
     "under two minutes" is the success condition an unexplained fourth verdict
     fails. Removing a customer-visible verdict is a change to what the product
     promises, so the call was the owner's, not this node's. Ruled as audited:
     no tile.
   - Decision: `OverviewModel` carries no `score` module, `GoalKey` drops
     `'score'`, and `readOverview` does not read the score at all — not read,
     not passed, no slot. A later surface cannot pick up a value that never
     reaches this model.
   - Alternatives considered: keep the tile without a goal — rejected,
     criterion 4 binds every module that leads with a number and a goalless
     verdict is the bare number it forbids; move the score to a smaller mark on
     the same screen (a header chip) — rejected, it is the same unexplained
     verdict in less space, and the ruling is about whether the number is on
     this screen; keep it and explain the composition — rejected, REQ-004's
     non-goals forbid that explanation on any surface, so it would be a
     requirement change dressed as a design one.
   - Consequences: the free report is untouched — BP-024's `Verdict` still
     carries the score and its band word, which is the verdict a stranger came
     for. `scans.score` keeps its writer and its check constraint (BP-024
     decision 4); this screen was never one of its readers, since the growth
     chart plots searches-you-appear-in (`BUILD.md` §4.5 item 2), not the score.
     BP-005 retires `GOAL_VALUES.score`. The tile row goes from three to two and
     how it lays out is design-guardian's (rule 7.3), as in decision 6.
   - Cost of reversing: one module, one goal key and one pin — cheap in code.
     What reversal costs the reader is what the ruling names: a verdict on the
     landing screen the product has decided never to explain.
...
None. The one that stood here — which of the AI tile's two readings survives —
was ruled by the owner on 2026-09-03 (`OWNER-QUESTIONS.md` item 7): the windowed
one, and decision 5a records the shape and the re-denominated goal. The
composite score's tile went with the same day's item 8 (decision 7). REQ-041
criteria 4 and 12 and JN-005 are the analyst's half of both.
...
- 2026-09-03 amended — architect — settled the `RivalGapModule` `REVIEW(conflict with PROJECT)` line: decision 6 added, recording the redundancy and routing which encoding is dropped to design-guardian's preview gate (rule 7.3), not decided here. The `aiAnswers` `REVIEW(conflict with REQ-041)` line is the blueprint-side twin of `OWNER-QUESTIONS.md` item 7 (a number a customer reads, the owner's): left standing, no second home minted. No `satisfies` or interface change.
- 2026-09-03 amended — architect — `/decide`, owner rulings on `OWNER-QUESTIONS.md` items 7 and 8 (both as audited): decision 5a added (the AI tile shows one reading — `citedWeeks` of `windowWeeks` as the headline; `GOAL_VALUES.ai_answers` 6 → 4, re-denominated to weeks of the window, derivation under rule 1.1) and decision 7 added (no composite-score tile: `OverviewModel.score` removed, `GoalKey` drops `'score'`, BP-005 retires `GOAL_VALUES.score`). Decisions 1, 2 and 5 annotated where they are superseded; two Error & edge bullets rewritten and one added; the `REVIEW(conflict with REQ-041)` line closed and deleted. No schema change and no `satisfies` change; REQ-041 criteria 4 and 12 and JN-005 are the analyst's.
