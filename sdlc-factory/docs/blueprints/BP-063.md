...
satisfies: [REQ-079]
covers: []
depends-on: [BP-062, BP-060, BP-015, BP-016, BP-002, BP-003, BP-005]
blocked-by: []
rests-on:
...
graph TD
  ...
  BP063 -.->|ADR-081| LIVENESS["unpublish is scoped by liveness"]
...
## Blocked-by — discharged 2026-08-31 by ADR-081
...
rejected readings are **ADR-081**; the discriminator is
...
load-bearing; the enforcement point is BP-002's row policy) and **ADR-081**
(unpublish is scoped by liveness).

## Status grounds (rule 3.2)
Self-certified `approved` per rule 3.2. Grounds: cut from REQ-079;
`satisfies: [REQ-079]` and nothing else; `code:` globs sit inside BP-017's and
overlap no sibling's. Its `blocked-by` on REQ-056 / BP-015 is discharged by
ADR-081. REQ-079 is `in-review` while the analyst folds the liveness scoping into
criterion 4's wording; the design that criterion now binds is settled and this
node's approval stands on it. If the analyst's rewording changes the behaviour
rather than the wording, this node returns to review with it.
