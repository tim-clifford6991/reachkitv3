...
- Narrow viewport: the sidebar is replaced by top tabs rendering the same
  `DESTINATIONS` tuple, and the domain block and publishing state collapse into
  the tab bar's header rather than disappearing (criterion 3 binds *any* app
  screen, so the publishing state is not a desktop-only affordance).
