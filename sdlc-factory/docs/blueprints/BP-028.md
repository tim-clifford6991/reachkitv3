...
blocked-by: []
rests-on:
  - claim: "Enough of a free scan's own twelve SERPs, or its suggestions call, are served from BP-008's 30-day cache to leave the 4.5c a complete correction needs inside CAPS.FREE_C 12c. A fully uncached scan ledgers 9.0c (ADR-094 decision 3 ledgers the async surcharge unconditionally, so its twelve flagged SERPs bill 4.8c whether or not the vendor refunds), which leaves 3.0c and truncates the correction's SERP fan-out. Nothing in the corpus measures the cache hit rate; only live traffic can. Raised 2026-09-03 with decision 4's amendment."
    disposition: open
supersedes: []
code:
...
- A correction's own cost and clock are BP-012's: it belongs to the scan it
  ...
  4a): **the scan itself is ~9.0¢** (0.6¢ two nano calls + 1.8¢ suggestions +
  4.8¢ twelve live SERPs at ADR-094's surcharge rate + 1.8¢
  `ranked_keywords`@50), and that is the *ledgered* figure on every uncached
  scan, not merely a worst case, because ADR-094 decision 3 records the
  surcharge unconditionally. **The correction is ~4.5¢** (1.8¢ suggestions +
  0.3¢ one nano + 2.4¢ twelve live SERPs, **un**flagged since ADR-094 decision
  5 — its twelve queries are new and none can be a cache hit). The pair is
  **13.5¢ against `CAPS.FREE_C` 12¢, both inside one `CostContext`** — down
  from 15.9¢ before the ruling, and still over. A complete correction therefore
  needs the scan it corrects to have ledgered ≤ 7.5¢, which means at least 1.5¢
  of that scan — four of its SERPs, or its suggestions call — served from
  BP-008's 30-day cache. Where that does not happen the correction degrades
  part-way through its SERP fan-out by the rule below; the front matter carries
...
4a. **The cap stays at 12¢ and the correction stops paying for asynchronous AI
   ...
   - Consequences, stated as arithmetic rather than as intent:
     ...
     - a complete correction is reachable whenever the corrected scan ledgered
       ≤ 7.5¢, and not otherwise — the residual the front matter's `rests-on`
       row carries. Under the pre-ruling figures no fresh scan left enough for
       a flagged correction to buy more than about two of its twelve SERPs;
       after it, an uncached scan leaves about four and a scan with any cache
       hits leaves the whole set. The lever narrows the failure, it does not
       remove it;
