...
- A correction's own cost and clock are BP-012's: it belongs to the scan it
  ...
  corrected category. Corrected 2026-09-03 (decision 4): **the scan itself is
  ~9.0¢ worst case** (0.6¢ two nano calls + 1.8¢ suggestions + 4.8¢ twelve
  live SERPs at ADR-094's surcharge rate + 1.8¢ `ranked_keywords`@50), **the
  correction ~6.9¢ worst case** (1.8¢ + 0.3¢ + 4.8¢ — its twelve queries are
  new and none can be a cache hit) — **15.9¢ against `CAPS.FREE_C` 12¢, both
  inside one `CostContext`.** Whether that total is acceptable as designed is
  `OWNER-QUESTIONS.md` item 9; see decision 4.
...
4. **The correction shares the scan's one ceiling and degrades exactly as the
   ...
   - **What is not decided here:** whether `CAPS.FREE_C` should move, whether
     the second pass should decline the surcharge flag, or whether degrading
     is the accepted outcome — a number a customer reads (the free report's
     stated cost envelope, or what the corrected card counts) changes under
     each, so the choice among them is `OWNER-QUESTIONS.md` item 9's, not
     restated here.
