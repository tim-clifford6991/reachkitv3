...
- [ ] REVIEW(conflict with ADR-094): the NFR budget's "re-runs steps 2–5 at
      ~4.2¢ (10.5¢ worst case) against `CAPS.FREE_C` 12¢" is `BUILD.md` §6.7
      step 5's arithmetic, computed at 0.2¢ a SERP and one nano call, and both
      operands have since moved. Taking the corpus's own current figures:
      **the scan** is 0.6¢ nano ×2 (BP-025 decision 2) + 1.8¢ suggestions +
      4.8¢ for twelve live SERPs at the surcharge rate (BP-025 NFR, worst case)
      + 1.8¢ `ranked_keywords`@50 = **9.0¢**; **the correction** re-runs steps
      2–5 against a corrected category, so its twelve queries are new and none
      of them can be a cache hit — 1.8¢ + 0.3¢ + 4.8¢ = **6.9¢**. REQ-094
      criterion 3 puts both inside one ceiling ("spends no further scan
      allowance and **opens no second spend ceiling**"), as this budget's own
      first sentence says. **15.9¢ against a 12¢ cap.** Even with no surcharge
      charged at all, the pair is 11.1¢ and the cap is missed by 0.9¢. The
      failure is not a bill — BP-007 degrades — it is that the correction
      REQ-094 promises stops mid-flight and REQ-094 criterion 7's "the
      correction did not complete" becomes the ordinary outcome for exactly the
      reports whose market was read wrongly. Which lever moves is
      `OWNER-QUESTIONS.md` item 9: the flag's scope on a re-run is what ADR-094
      settled for the first pass and not for the second.
