...
meet.** The terminality of all five causes, the fifth cause itself, `VerifyNote`
and the WordPress population all derive from **REQ-063, `status: in-review`**,
and read **REQ-062** and **REQ-060** (`in-review`, both). All three carry **zero
open `REVIEW(...)` lines** after two full review rounds (rule 3.4), so the text
is settled — but §3's gate reads *no blueprint from a non-approved requirement*,
