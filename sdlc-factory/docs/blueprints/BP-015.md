 ...
 *  two implementations are BP-058's and BP-048's, and that the adapter — never a
 *  `kind === 'hosted'` test at a caller — carries the **two** booleans that
 *  decide who is in which population, so a third adapter cannot join either by
 *  defaulting into it (decision 3 below):
 *
 *  - `servesPublicly` — the destination makes the page publicly readable at an
 *    address we can fetch. The population boundary for REQ-062's 24-hour
 *    verification and REQ-063 criterion 1's weekly verdicts. **True at both
 *    destinations since ADR-084**, which is how WordPress joined both
 *    populations with no rule written anywhere.
 *  - `hostedByUs` — ReachKit runs this destination and can take the page off it.
 *    The boundary for REQ-056 criterion 15's `removed` arm against criterion
 *    16's `returned_to_draft`. **True for hosted only.**
 *
 *  **ADR-084 Decision 2 — these were one boolean until 2026-09-01 and must never
 *  be merged back.** They had the same value at every destination that existed,
 *  which is why they were one field; they now differ at WordPress, and
 *  collapsing them either stops verifying live customer pages or points
 *  `unpublish` at a delete call against a site ReachKit does not own. */
export type { DestinationAdapter, DeliveryResult, UnpublishResult } from './types'
//  UnpublishResult has five arms as of ADR-082, carried forward unchanged in
//  shape by ADR-084 with every arm's population moved; the shape is BP-045's and
//  is not restated here for the reason the comment above gives.
```

## Data model delta
**No migration glob is claimed here as of 2026-09-01.** This node declared
`supabase/migrations/*_publications*.sql` and
`supabase/migrations/*_destinations*.sql`, and both were declared *identically*
by the leaf that specifies the columns — BP-045 for `publications`, BP-058 for
`destinations`. structure.md rule 2a makes an identical pair a conflict under
rule 7.1 rather than a precedence: "the node that specifies the thing keeps the
glob and the node that merely reads it withdraws". Both are withdrawn here, on
the same subtractive resolution the `*_leads*.sql` / `*_suppressions*.sql` case
between BP-016 and BP-029 already took. Nothing about the schema changes; what
changes is that each migration file has one owner instead of two, which is what
rule 2's "one owner per path" requires and what `file → blueprint` (rule 5.6)
needs in order to answer at all.

What this node states about the schema, and keeps stating, is the *shape* the
subsystem depends on — the reasoning, not the columns (rule 5.2):

`drafts.state`, `drafts.veto_deadline`, `drafts.scheduled_for`; `publications` as
`BUILD.md` §10 with a unique index on `(draft_id, destination_id)` — the
idempotency guarantee is the index, not the code path — plus `delivery_state`,
`failure_reason`, `made_live_by_us` and `unpublish_outcome`, all specified in
**BP-045** (and `live_url` and `verify_due_at` now written for **every**
delivery at both destinations — ADR-084 Decision 2), and **no `verdict` column**. `BUILD.md` §10 lists
`verdict(working/too_early/not_working)` there; one column cannot answer REQ-063
criterion 3's previous measurement, criterion 4's interval, or criterion 6's "the
date of the last verdict it did receive", and its enum has no fourth value for
`not_judgeable`. A page's standing is a row per page per week in **BP-051's
`page_verdicts`**, unique on `(publication_id, week_start)`, insert-only. BP-051
decision 1 is the home of that reasoning; the job of whichever migration carries
`publications` is to not create the second one, and a test asserts the column's
absence, because adding it back reads as a denormalisation win and would give the
product two answers to "did this page work".

`destinations` with `config jsonb` (credentials encrypted at rest, never logged),
`health`, `health_reason`, `last_checked_at` and `stamp_capable`, all specified
in **BP-058**. **No foreign key into `publications` carries `on delete
cascade`** from `drafts`, `destinations` or `sites` — ADR-080 and ADR-051 both
turn on that absence, and the whole-account purge is the only path permitted to
delete a `publications` row.
...
- **A WordPress delivery reaches `published` in exactly the sense every other
  delivery does** (ADR-084 Decision 1, REQ-060 criterion 2): one create call
  publishes it live, the record carries the permalink the site returned, and the
  page is verified at 24 hours and judged weekly like any other. This bullet said
  the opposite until 2026-09-01, on REQ-060 criterion 1 as it then stood; that
  criterion has inverted. A **destination whose credential can create but not
  publish** is a destination state (`failing`, reason `cannot_publish`) and never
  a silently drafted post — BP-058's probe, ADR-084 Decision 3 and ADR-086.
...
- Unpublish is available for as long as a page is published. Which family of
  arms applies is read from `hostedByUs`, never from `servesPublicly`, which is
  now true at both destinations: on the destination ReachKit runs the page is
  removed and its address returns 410 (BP-004). At the customer's own WordPress
  the product stops treating it as live in every case, and what it does inside
  their site is **scoped by liveness** (**ADR-082**, superseded by **ADR-084**,
  which carries the scoping forward and inverts the populations): a post ReachKit
  itself made live is returned to draft there — **now every post it created**; a
  post it created but never made live is named for the customer and not touched —
  **now no post at all, and the arm is kept anyway**; a post no longer there is
  reported as gone with nothing to remove; and a site that could not be reached
  leaves the post untouched with a retry offered on the page's record. All four
  arms are BP-048's and the union is BP-045's.
...
3. **Two destination adapters behind one interface, no plugin registry** — Status: accepted
   ...
   - The adapter also declares the population booleans. Expressed as a
     `kind === 'hosted'` test at each caller they would default a third adapter
     into a population; expressed on the adapter, a third adapter cannot ship
     without answering both questions. BP-051 rested on this and could not add it.
   - **Amended by ADR-084 Decision 2 (2026-09-01): one boolean became two.**
     `servesPublicly` kept its name and changed value at WordPress; `hostedByUs`
     is new and carries the fact `servesPublicly` used to carry alongside it.
     Alternative 4 of that file records why the obvious cheaper move — keep one
     boolean and read the unpublish arm from `kind === 'hosted'` — was rejected:
     a `kind` test is precisely the rule this decision exists not to write, and
     it re-adds a destination-kind switch every future destination must be
     threaded through. The merge back is the deletion ADR-084's warning block
     forbids; it reads as removing a redundant flag and fails no test.
...
**Status grounds addendum, 2026-09-01 (rule 3.2).** The edits of 2026-09-01
(ADR-084's boolean split, the WordPress delivery reaching `published` in the
ordinary sense, and the inverted unpublish populations) restate what BP-045,
BP-048, BP-049 and BP-058 specify, and those nodes derive from **REQ-056,
REQ-060 and REQ-062 — `status: in-review`, all three**, settled after two review
rounds with zero open `REVIEW(...)` lines but not yet signed. §3's gate binds a
blueprint to an approved *ancestor* and this node has none, so the gate is
literally silent here — but that is a technicality and not a licence: what this
node now says about WordPress is true only if the owner ratifies REQ-060
criterion 1's inversion. Stated plainly rather than rested on the empty
`satisfies` edge. The leaves' own addenda carry the same assertion where the gate
does bind.

## Open questions
None. The one that stood here — REQ-056 criterion 16 against REQ-079 criterion 4
— was ruled on 2026-08-31 and is now **ADR-082**, which supersedes ADR-081 and
carries the liveness scoping forward over criterion 16's four outcomes. BP-048's
`blocked-by` and BP-063's are discharged with it.
