...
- [ ] REVIEW(conflict with BP-025): the NFR budget reads "≤ `CAPS.FREE_C` (12¢)
      ledgered, against `BUILD.md` §6.3's expected ~6.3¢, **so the cap is
      headroom rather than the operating point**". Both halves have moved. The
      6.3¢ is superseded twice — BP-011's NFR budget corrects it to 6.6¢ on the
      two-nano-call reading, and since ADR-094 BP-025's own worst case is 7.2¢
      for that node alone, 9.0¢ with F4's `ranked_keywords`@50. And the
      conclusion is now the wrong way round: with the one correction REQ-094
      promises inside the same ceiling (criterion 3), the worst case is 15.9¢
      and the typical corrected report 11.1¢ — see BP-028's line. This node
      owns the free scan's ceiling and its budget is where a planner reads
      whether that ceiling binds; it currently says it does not.
