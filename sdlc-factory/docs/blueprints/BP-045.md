...
- **`retryUnpublishWrite` takes no edge at all.** `unpublished` is terminal
  (c3, and it is in `TERMINAL`), so there is no sixteenth transition and the
  retry never asks for one: it calls the adapter, rewrites `unpublish_outcome`,
  and returns. A discriminating test asserts that a retry leaves
...
meet.** This node's edits of 2026-09-01 derive from **REQ-056, `status:
in-review`** — criterion 6 rewritten to split on the page's current state, and
criterion 16 unchanged but with its populations inverted by ADR-084 — and read
**REQ-060** and **REQ-062** (`in-review`, both) for `live_url` at WordPress and
for criterion 7's carried outcome. All three carry **zero open `REVIEW(...)`
...
the design, not on the gate, and this node does not claim otherwise.

What the two rulings changed here is bounded and worth stating: two column
meanings, one boolean split on the adapter, one added module (`record/`), and
**no state, no transition and no guard** — REQ-056 criterion 1's ten states and
criterion 2's fifteen transitions are untouched by both. If the owner's
signature changes what any of those criteria promise rather than ratifying them,
the librarian reverts this status retroactively and names the clause. No
`blocked-by` is added: an `in-review` requirement mid-gate is not a conflict with
another artifact (rule 2.3).
