...
after which no later visit ever returns them to the wait.
...
3. **The degraded sentence is a projection of the current report, never a stored flag** — Status: accepted
   - Context: criterion 3 requires one written sentence about what could not be
     ...
     afterwards, and REQ-029's own open `REVIEW(gap)` notes that nothing states
     when it stops.
   ...
   - Consequences: this answers the requirement's open gap without changing what
     is promised; it is reported to the analyst rather than written into REQ-029,
...
Self-certified `approved`. Grounds: cut from REQ-029, which carries
`status: approved`; `satisfies: [REQ-029]` is the derivation edge. Its engine
