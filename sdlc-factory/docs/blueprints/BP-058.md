...
meet.** REQ-074 is still `approved` and is still this node's `satisfies`
ancestor, so §3's gate is met for everything this node held before today. It is
**not** met for the 2026-09-01 additions — `publish_capable`, the
`cannot_publish` reason, the `reconnect_other_account` action and the three
prohibitions on that occasion's line — which derive from **REQ-060 criterion 7,
`status: in-review`**. That text is settled (two review rounds, zero open
`REVIEW(...)` lines, rule 3.4) and unsigned, and this node does not claim
otherwise; the `approved` status covers them under rule 3.2 on the design, and
the librarian may revert retroactively and name the clause.

There is a second, sharper thing to say about them: **REQ-074 owes a criterion
for this occasion and does not have one.** REQ-060 criterion 7 narrowed its own
...
2.3). It is the analyst's to close.
