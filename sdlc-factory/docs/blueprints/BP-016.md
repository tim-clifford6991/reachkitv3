...
1. **The register is a typed constant in this node, not prose in a requirement** — Status: accepted
   ...
   - Alternatives considered: a documentation table — rejected, it cannot make an
     ...
     2.4.
   - Consequences: REQ-064 carries three open `REVIEW` lines about this exact
     seam (see Open questions). The register is built to the requirement's
     non-goal as written; if `/decide` moves the rule back into a criterion, the
     constant becomes its projection rather than its home.
...
## Open questions
- [ ] REQ-064 carries three unresolved `REVIEW` lines whose subject is this
      seam — criterion 6 versus REQ-057 criterion 7's unsuppressible send;
      three requirements citing text criterion 6 no longer carries; and the
      double line on a partly measured week. The register above is built to the
      requirement as written and the `unsuppressibleWhen` field records the
      exception explicitly, but the feature node that satisfies REQ-064 carries a
      `blocked-by` until those three are ruled.
