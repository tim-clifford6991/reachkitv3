...
recorded that the `DestinationClause`, the asynchronous `tellingFor` and
decision 5 derived from **REQ-057 criterion 9** while it was `status:
in-review`, reading **REQ-060 criterion 7** in the same state, and that §3's
gate was therefore not met. **The owner has since approved both, and criterion
9's three arms are the three this node renders. The gate is now met and these
grounds hold.** No retroactive refusal is owed.

**The deferred rename was already done, and the addendum was wrong to say
otherwise.** The same addendum recorded a rename of `wholeOfTheTelling` to
`isUnsuppressible` as "not made in this pass" and as "a change to this node's
interface and to WO-216". Both halves were false when written: this node's `##
Public interface` spells the function `isUnsuppressible`, and so does WO-216 —
in its file plan, its `## Interfaces` block and its two REQ-057 criterion rows.
WO-216's `## Out of scope` reported the discrepancy back rather than acting on
it, which was right. Nothing bears the old name anywhere in the corpus. The
paragraph is discharged as **already true**, not deleted as unwanted: the
premise the analyst struck from criteria 7 and 9 is struck, the name no longer
refers to it, and the three behaviours the flag drives are the same three they
were.
