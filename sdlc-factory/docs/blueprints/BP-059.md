...
writes `users.notify`, which ships in BP-002's baseline (that node's data-model
delta already allocates "`users.notify jsonb` (REQ-075)").
...
graph TD
  ...
  BP059 --> BP002["BP-002 · users.notify"]
...
## Data model delta
`users.notify jsonb` — BP-002's baseline holds the column; this node holds its
shape and its meaning. The value is a sparse object over `NotifyKind`, default
...
No migration topic is claimed. `structure.md` rule 3 fixes the topic-token list,
`users` is BP-002's baseline territory, and three sibling requirements
(REQ-075, REQ-077, REQ-079) each add columns to the same table — so every
user-column delta rides BP-002's baseline rather than minting a token this map
does not carry.
...
4. **This node owns no migration** — Status: accepted
   - Context: `structure.md` rule 3 fixes the topic-token list and gives every
     other node a token of its own; there is no token for a user-column change,
     `users` is the baseline's, and three sibling requirements add columns to
     `users` concurrently.
   - Decision: `users.notify` ships in BP-002's baseline, which already names it;
     `code:` here claims no `supabase/migrations/**` glob.
