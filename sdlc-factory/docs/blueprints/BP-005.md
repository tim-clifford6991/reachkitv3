...
- [ ] REVIEW(conflict with RESEARCH-dataforseo-endpoints): `PRICE_BOOK`
      carries `AI_MODE_LIVE_C: 0.2; AI_MODE_STD_C: 0.06`, transcribed from
      `BUILD.md` §6.1. `registry/evidence/RESEARCH-dataforseo-endpoints.md` §3
      quotes the vendor's own pricing page, accessed 2026-09-03: AI Mode is
      *"Live Mode … $0.004"* and *"Standard Queue … $0.0012"* — **0.4¢ · 0.12¢,
      twice the pinned value**, and its §6 divergence table marks this the one
      row of nine that "diverges ×2". That file names the correction as
      "a price-book row and is the system's to make (rule 1.1, BP-005)" and it
      has not been made. Derived consequences, none of which reaches a cap or a
      customer-visible number: the paid weekly battery is 12 × (0.12 + 0.12) =
      **2.88¢, not the 2.2¢** BP-008's NFR budget and `DATA-COSTS.md` §3 state;
      the weekly refresh is ≈8.7¢ against `CAPS.WEEKLY_C` 40¢, and BP-050's NFR
      budget still reads "≈8¢ ledgered per site"; the deep pass is ≈30.7¢
      against `CAPS.DEEP_C` 150¢. Two pins and three NFR lines, one edit each.
- [ ] REVIEW(gap): ADR-094 decision 3 makes BP-008 pass "the surcharge rate (2x
      the base price)" as `recordFetch`'s `costCents` on every flagged SERP, and
      **`PRICE_BOOK` has no row for it**. `DATA-COSTS.md` §6 item 5 — "each
      needs its own price-book row before it ships" — and the charter's repo
      table ("**Every** pinned number in the specification") both put it here.
      The value is not a transcription either: evidence §2.3 records the vendor
      disagreeing with itself, the docs saying *"you will be charged extra
      $0.002"* and the help centre's worked example and the pricing page saying
      one base price (*"a Standard request … costs $0.0006. If this request
      contains \"load_async_ai_overview\": \"true\", you will be billed an
      additional $0.0006"*). ADR-094's "2x the base price" takes the second
      reading without saying it chose; on the standard queue the two readings
      differ by 3.3× per call (0.12¢ against 0.26¢). Pin it, name which reading
      it takes, and `tests/pins.test.ts` can quote the page it came from.
- [ ] REVIEW(gap): `PRICE_BOOK` carries six DataForSEO rows and **no inference
      row at all**, while BP-009's `llm()` must supply a `costCents` to
      `recordFetch` for eight call sites across two tiers and BP-007 enforces
      every cap from those figures. `DATA-COSTS.md` §1 states the prices (nano
      "$0.20 · $1.25", Haiku 4.5 "$1.00 · $5.00" per M in · out) and §6 names
      model creep as danger 4, enforced "in the seam, not assumed" — a seam that
      cannot price a call cannot cap it, and `CAPS.DRAFT_C` 45¢ is enforced over
      six model calls whose prices this file does not hold. Rows for the two
      shipped tiers, and the token budgets `DATA-COSTS.md` §4 already states per
      step, are what the assertion would quote.
