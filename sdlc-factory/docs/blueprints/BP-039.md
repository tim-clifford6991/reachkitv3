...
meet.** `WayThrough`, `DayDetail.record` and decision 4 derive from **REQ-043
criterion 12, `status: in-review`** — rewritten whole on 2026-09-01, not
appended to — and read **REQ-062 criteria 4 and 7** and **REQ-056 criteria 6 and
7** (`in-review`, both requirements). All three carry **zero open `REVIEW(...)`
...
Criterion 12 is **new promise with no work order behind it**: the day-panel work
orders that exist predate it, and the way into the customer's own WordPress has
none at all. That is the planner's, and it is stated here so it is not read as
covered. If the owner's signature changes criterion 12 rather than ratifying it,
this status reverts by the librarian's retroactive refusal. No `blocked-by` (rule
2.3); the WordPress-admin-address assumption is the second `rests-on` row above,
`open`.
