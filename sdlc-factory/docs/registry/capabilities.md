...
| Ten states, fifteen transitions, no duplicate post, instant stop | `src/lib/publish/machine/`, `/attempt/`, `/record/` | BP-045 | `TRANSITIONS` · `transition()` · `publish()` · `unpublish()` · `retryUnpublishWrite()` · `setPublishing()` · `heldPages()` · `pageRecordFor()` | BP-001, BP-003, BP-039, BP-044 |
